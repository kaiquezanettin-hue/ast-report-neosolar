...
function renderProdutos(d) {
  ...
  if (allLocations.length === 0) {
    const ctx = document.getElementById('chart-test-location');
...
function renderOperacao(d) {
  ...
  for (const h of Object.values(deskHistory)) {
    ...
    // Só processa tickets que existem no RMA
    // ticketNumber pode ser string com prefixo 'rma_' para tickets antigos
    const ticketNum = String(h.ticketNumber || '').replace('rma_', '').trim();
    const rma = rmaByDesk[ticketNum];
    if (!rma) continue;
    ...
    // Dados do RMA para cruzamento (já garantido acima)
    const linha = rma.fornecedor || rma.fornecedorNome || 'Não identificado';
    console.log('ticket:', h.ticketNumber, 'rma:', JSON.stringify(rma));
    ...
    plugins: [{
      ...
      afterDatasetsDraw(chart) {
        ...
        chart.getDatasetMeta(0).data.forEach((bar, i) => {
          ...
          const x = bar.x - 6;
          const y = bar.y;
          // Só mostra dentro da barra se houver espaço
          if (bar.width > 60) {
