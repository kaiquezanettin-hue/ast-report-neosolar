...
/* ── MAIN ── */
main { padding: 24px; width: 100%; max-width: 100%; box-sizing: border-box; overflow-x: hidden; }
.tab-content.fullwidth { margin: 0; padding: 0; width: 100%; }
.tab-content.fullwidth .panel { border-radius: 4px; }
.tab-content.fullwidth table { width: 100%; }
.tab-content.fullwidth th:first-child, .tab-content.fullwidth td:first-child { width: 35%; min-width: 200px; }
...
@media (max-width: 900px) { #histograma-grid { grid-template-columns: 1fr 1fr !important; } }
@media (max-width: 600px) { #histograma-grid { grid-template-columns: 1fr !important; } }
@media (max-width: 600px) {
  header { padding: 0 12px; height: 48px; }
  header h1 { font-size: 14px; }
  .header-sep { display: none; }
  main { padding: 12px; }
  .period-bar { padding: 8px 12px; gap: 8px; }
  .tabs { padding: 0 12px; }
  .kpis { gap: 8px; }
  .kpi { min-width: 80px; padding: 10px 12px; }
  .kpi .val { font-size: 22px; }
  .panel { padding: 12px; }
  .tab-content.fullwidth th:first-child, .tab-content.fullwidth td:first-child { min-width: 140px; }
...
.chart-wrap, .chart-wrap.tall { overflow: hidden; }
</style>
</head>
<body>
...
<div id="tab-produtos" class="tab-content">
  ...
  <!-- Gráficos de serviço por período -->
...
<div id="tab-operacao" class="tab-content">
  ...
  <!-- KPIs de custo -->
  <div class="kpis" style="margin-bottom:16px;" id="kpi-custo"></div>
    ...
    <div style="overflow-x:auto;max-height:450px;overflow-y:auto;">
      <table class="tbl" id="tbl-custo-tickets" style="width:100%;">
        <thead><tr>
          ...
          <th>Fornecedor</th>
          <th>Garantia</th>
          <th class="num">H. Úteis</th>
          <th class="num">Custo (R$)</th>
          <th>Status de custo</th>
          <th>Data</th>
        </tr></thead>
...
<div id="tab-sla" class="tab-content fullwidth">
  ...
  <!-- Filtro de garantia + KPIs -->
  <div style="display:flex;align-items:center;gap:16px;margin-bottom:16px;flex-wrap:wrap;">
    <div style="display:flex;align-items:center;gap:8px;">
      <label style="font-size:12px;color:var(--gray2);">Tipo:</label>
      <select id="filtro-garantia" onchange="renderSlaReport()" style="background:var(--bg3);border:1px solid var(--border);color:var(--white);padding:6px 10px;border-radius:4px;font-family:var(--font);font-size:13px;">
  ...
  <!-- Tabela de lead time por status -->
  <div class="panel yellow" style="margin-bottom:16px;">
    <div class="panel-title">Lead Time Médio por Status <span>tickets fechados no período</span><span style="margin-left:auto;color:var(--yellow);font-size:11px;cursor:pointer;font-weight:600;" onclick="abrirValidacao()">↗ VER DETALHES POR TICKET</span></div>
  ...
  <!-- Tabela de validação por ticket -->
  <div class="panel" style="margin-bottom:16px;" id="painel-validacao" style="display:none;">
    <div class="panel-title" style="display:flex;justify-content:space-between;align-items:center;">
      Detalhes por Ticket — Validação
      <div style="display:flex;gap:8px;align-items:center;">
        <select id="validacao-status" onchange="renderValidacaoTabela()" style="background:var(--bg3);border:1px solid var(--border);color:var(--white);padding:3px 8px;border-radius:4px;font-family:var(--font);font-size:12px;">
          <option value="">Todos os status</option>
        </select>
        <button onclick="document.getElementById('painel-validacao').style.display='none'" style="background:var(--bg3);border:1px solid var(--border);color:var(--gray2);padding:3px 10px;border-radius:4px;cursor:pointer;font-size:11px;">Fechar</button>
      </div>
    </div>
    <div style="overflow-x:auto;max-height:400px;overflow-y:auto;">
      <table class="tbl" id="tbl-validacao" style="width:100%;font-size:11px;">
        <thead><tr>
          ...
          <th>Status</th>
          <th>Macro</th>
          <th class="num">Horas Brutas</th>
          <th class="num">Dias Úteis</th>
          <th class="num">SLA (du)</th>
  ...
  <!-- Histograma por macro status — cards separados -->
  <div class="panel-title" style="margin-bottom:12px;font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:600;letter-spacing:0.8px;text-transform:uppercase;color:var(--gray2);">
    Histograma de Lead Time por Macro Status <span style="color:var(--yellow);font-size:11px;font-weight:400;margin-left:8px;">dias úteis — período selecionado</span>
  </div>
  <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:16px;" id="histograma-grid">
    <div class="panel" style="border-left-color:var(--green);padding:12px;">
      <div class="panel-title" style="margin-bottom:8px;">ENTRADA</div>
      <div style="position:relative;height:220px;"><canvas id="chart-hist-entrada"></canvas></div>
    </div>
    <div class="panel" style="border-left-color:var(--blue);padding:12px;">
      <div class="panel-title" style="margin-bottom:8px;">AVALIAÇÃO</div>
      <div style="position:relative;height:220px;"><canvas id="chart-hist-avaliacao"></canvas></div>
    </div>
    <div class="panel" style="border-left-color:#e0c052;padding:12px;">
      <div class="panel-title" style="margin-bottom:8px;">PAGAMENTO</div>
      <div style="position:relative;height:220px;"><canvas id="chart-hist-pagamento"></canvas></div>
    </div>
    <div class="panel" style="border-left-color:var(--orange);padding:12px;">
      <div class="panel-title" style="margin-bottom:8px;">TÉCNICO</div>
      <div style="position:relative;height:220px;"><canvas id="chart-hist-tecnico"></canvas></div>
    </div>
    <div class="panel" style="border-left-color:var(--purple);padding:12px;">
      <div class="panel-title" style="margin-bottom:8px;">DEVOLUÇÃO</div>
      <div style="position:relative;height:220px;"><canvas id="chart-hist-devolucao"></canvas></div>
    </div>
  </div>
...
function switchTab(id) {
  ...
  // Re-renderiza ao ativar aba (canvas precisa estar visível para ter dimensões)
  if (id === 'produtos' && window._lastReportData) {
    setTimeout(() => renderProdutos(window._lastReportData), 80);
  }
  if (id === 'sla' && typeof renderSlaReport === 'function') {
    setTimeout(() => renderSlaReport(), 80);
  }
  if (id === 'operacao' && window._lastReportData) {
    setTimeout(() => renderOperacao(window._lastReportData), 80);
  }
}
...
// Plugin global para texto no centro do donut
Chart.register({
  id: 'centerText',
  beforeDraw(chart) {
    if (chart.config.options?.centerText) {
      ...
      ctx.save();
      const txt1 = chart.config.options.centerText.line1;
      const txt2 = chart.config.options.centerText.line2;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      const cx = width / 2, cy = height / 2;
      ctx.font = 'bold 32px Barlow Condensed, sans-serif';
      ctx.fillStyle = '#FFFDF0';
      ctx.fillText(txt1, cx, cy - 10);
      ctx.font = '11px Barlow, sans-serif';
      ctx.fillStyle = '#888888';
      ctx.fillText(txt2, cx, cy + 16);
      ctx.restore();
    }
  }
});
...
// ─── Desk tickets (paginado no frontend) ─────────────────────────────
let deskTickets = [];
let deskLoaded = false;

// ─── Histórico de status por ticket ──────────────────────────────────
// { ticketId: { statusTimes: { statusName: hours }, totalHours } }
let deskHistory = {};
let historyLoading = false;
let historyLoaded = false;

// Mapeamento de status para grupos (igual ao report de referência)
const STATUS_GRUPOS = {
  // ENTRADA
  'Aguardando 1º contato - AST': 'ENTRADA',
  'Pendencia AST - SAC': 'ENTRADA',

  // AVALIAÇÃO
  'Aguardando teste': 'AVALIAÇÃO',
  'Em teste': 'AVALIAÇÃO',
  'Ag. teste': 'AVALIAÇÃO',
  // TÉCNICO
  'Aguardando manutençao (SG)': 'TÉCNICO',
  'Em manutençao': 'TÉCNICO',
  'Em manutenção': 'TÉCNICO',
  'Aguardando laudo': 'TÉCNICO',
  'Aguardando RMA': 'TÉCNICO',
  'Ag. Peça Reposição': 'TÉCNICO',
  'Aguardando Peça Reposição': 'TÉCNICO',
  // PAGAMENTO
  'Aguardando aprovaçao de manutençao (SG)': 'PAGAMENTO',
  'Ag. aprovação manutenção (SG)': 'PAGAMENTO',
  // DEVOLUÇÃO (sem Descarte e Produto despachado AST)
  'Em tratativa p/ devoluçao cliente': 'DEVOLUÇÃO',
  'Em tratativa p/ devolução cliente': 'DEVOLUÇÃO',
  'Aguardando coleta': 'DEVOLUÇÃO',
  'Ag. Prazo / Autorização Descarte': 'DEVOLUÇÃO',
  'Aguardando Prazo / Autorização Descarte': 'DEVOLUÇÃO'
  // IGNORADOS: Aguardando Chegada de Produto na Neosolar, Descarte, Produto despachado AST
};

// SLA em dias úteis por status
const SLA_DIAS_UTEIS = {
  'Pendencia AST - SAC': 2.0,
  'Aguardando 1º contato - AST': 0.5,
  'Aguardando teste': 3.0,
  'Em teste': 0.25,
  'Em manutençao': 0.25,
  'Em manutenção': 0.25,
  'Aguardando aprovaçao de manutençao (SG)': 2.0,
  'Ag. aprovação manutenção (SG)': 2.0,
  'Aguardando manutençao (SG)': 1.0,
  'Aguardando laudo': 0.13,
  'Aguardando RMA': 0.5,
  'Ag. Peça Reposição': 0.5,
  'Aguardando Peça Reposição': 0.5,
  'Em tratativa p/ devoluçao cliente': 2.0,
  'Aguardando coleta': 1.0
};

// ─── Cálculo de horas úteis reais (09h–18h, seg–sex, sem feriados) ──
...
function getFeriados(ano) {
  if (!_feriadosCache[ano]) _feriadosCache[ano] = getFeriadosBR(ano);
  return _feriadosCache[ano];
}

const HORA_INICIO = 9;   // 09h
const HORA_FIM = 18;     // 18h
const HORAS_DIA_UTIL = HORA_FIM - HORA_INICIO; // 9h por dia

function isDiaUtil(date) {
  const dow = date.getDay();
  if (dow === 0 || dow === 6) return false;
  const dateStr = `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;
  return !getFeriados(date.getFullYear()).has(dateStr);
}

// Calcula horas úteis entre dois momentos (minuto a minuto seria pesado — usa lógica por dia)
function horasUteisEntre(inicio, fim) {
  ...
  // Avança para início do dia atual
  while (cur < fim) {
    const diaAtual = new Date(cur);
    ...
    // Início do expediente desse dia
    const expInicio = new Date(diaAtual);
    expInicio.setHours(HORA_INICIO, 0, 0, 0);

    // Fim do expediente desse dia
    const expFim = new Date(diaAtual);
    expFim.setHours(HORA_FIM, 0, 0, 0);

    // Interseção entre [cur, fim] e [expInicio, expFim]
    const segInicio = Math.max(cur.getTime(), expInicio.getTime());
    const segFim = Math.min(fim.getTime(), expFim.getTime());

    if (segFim > segInicio) {
      total += (segFim - segInicio) / 3600000;
    }
...
// Retorna dias úteis (1 dia = 9h úteis)
function horasParaDiasUteis(horas, startTime) {
  if (!startTime || horas <= 0) return 0;
  const inicio = new Date(startTime);
  const fim = new Date(inicio.getTime() + horas * 3600000);
  const horasUteis = horasUteisEntre(inicio, fim);
  return horasUteis / HORAS_DIA_UTIL;
}

// Status ignorados no histórico e no cálculo de SLA
...
// Status que indicam ticket "concluído" para o report
...
// Busca histórico de tickets concluídos em background
async function fetchDeskHistory(tickets) {
  if (historyLoading) return;
  historyLoading = true;
  historyLoaded = false;

  // Mapa ticketNumber → ticket object (tickets carregados do Desk)
  const ticketMap = {};
  for (const t of tickets) ticketMap[String(t.ticketNumber)] = t;

  // RMA filtrado a partir de 01/01/2025
  const rmaRaw = window._rmaRaw || [];
  const dataCorte = new Date('2025-01-01');

  const rmaFiltrado = rmaRaw.filter(r => {
    const d = parseDataBR(r.addedTime);
  ...
  // Separa: tickets disponíveis no Desk vs tickets antigos só no RMA
  const comId = [];       // têm ticketId disponível
  const semId = [];       // só têm ticketNumber, precisam buscar via Search API

  for (const num of rmaNumeros) {
    const cacheKey = 'rma_' + num;
    if (deskHistory[cacheKey] || deskHistory[ticketMap[num]?.id]) continue;
    if (ticketMap[num]) comId.push(ticketMap[num]);
    else semId.push(num);
  }

  // Também adiciona tickets Desk em status de conclusão não presentes no RMA
  for (const t of tickets) {
    const num = String(t.ticketNumber);
    if (!rmaNumeros.includes(num) && (t.closedTime || STATUS_CONCLUIDOS.has(t.status)) && !deskHistory[t.id]) {
      comId.push(t);
    }
  }

  const totalParaBuscar = comId.length + semId.length;
  let processed = 0;
  document.getElementById('last-update').textContent = `⏳ Histórico: 0/${totalParaBuscar} tickets...`;

  // 1. Busca por ticketId (rápido — já temos o ID)
  for (const t of comId) {
    ...
    try {
      ...
      if (!data.error && data.statusTimes && Object.keys(data.statusTimes).length > 0) {
        deskHistory[t.id] = {
          statusTimes: data.statusTimes,
          totalHours: t.totalHours || 0,
          ticketNumber: t.ticketNumber,
          assigneeName: t.assigneeName,
          createdTime: t.createdTime,
          closedTime: t.closedTime || t.modifiedTime
        };
      }
    } catch {}
    processed++;
    if (processed % 10 === 0) {
      document.getElementById('last-update').textContent = `⏳ Histórico: ${processed}/${totalParaBuscar} tickets...`;
      renderSlaReport();
      if (document.getElementById('tab-operacao')?.classList.contains('active') && window._lastReportData) {
        renderOperacao(window._lastReportData);
      }
    }
  }

  // 2. Busca por ticketNumber (tickets antigos via Search API)
  for (const num of semId) {
    const cacheKey = 'rma_' + num;
    if (deskHistory[cacheKey]) { processed++; continue; }
    try {
      const res = await fetch(`/api/desk-history?ticketNumber=${num}`);
      const data = await res.json();
      if (!data.error && data.statusTimes && Object.keys(data.statusTimes).length > 0) {
        deskHistory[cacheKey] = {
          statusTimes: data.statusTimes,
          totalHours: 0,
          ticketNumber: num,
          assigneeName: 'Sem agente',
  ...
  renderSlaReport();
  // Re-renderiza operação se estiver ativa
  if (document.getElementById('tab-operacao')?.classList.contains('active') && window._lastReportData) {
    renderOperacao(window._lastReportData);
  }
}

// Renderiza tabela e gráfico de SLA com base no histórico
function renderSlaReport() {
  const rmaData = state.report?.rma;
  const filtroGarantia = document.getElementById('filtro-garantia')?.value || 'todos';

  // Mapa ticket# → tipo de garantia via RMA
  const rmaMap = {};
  if (rmaData) {
    // Precisamos dos dados raw do RMA — buscamos do cache do report
  ...
  // Filtra tickets do histórico pelo período e tipo de garantia
  ...
  const histEntries = Object.values(deskHistory).filter(h => {
    // Só inclui tickets que têm statusTimes relevantes (passaram por status de análise)
    if (!h.statusTimes || Object.keys(h.statusTimes).length === 0) return false;
    const d = new Date(h.createdTime);
  ...
  const total = histEntries.length;

  // Agrupa por status — conta tickets únicos por status
  const statusMetrics = {}; // { status: { totalDias, ticketsUnicos: Set, dentroSla } }
  for (const h of histEntries) {
    for (const [status, hours] of Object.entries(h.statusTimes || {})) {
      ...
      const dias = horasParaDiasUteis(hours, h.createdTime);
      const slaDias = SLA_DIAS_UTEIS[status];
      statusMetrics[status].totalDias += dias;
      statusMetrics[status].ticketsUnicos.add(h.ticketNumber);
      if (!slaDias || dias <= slaDias) statusMetrics[status].dentroSla++;
    }
  }

  // Monta tabela agrupada
  const grupos = ['ENTRADA', 'AVALIAÇÃO', 'PAGAMENTO', 'TÉCNICO', 'DEVOLUÇÃO'];
  const tblEl = document.getElementById('tbody-sla-hist');
  if (!tblEl) return;

  let rows = '';
  for (const grupo of grupos) {
    const statusDoGrupo = Object.entries(statusMetrics).filter(([s]) => STATUS_GRUPOS[s] === grupo);
    if (statusDoGrupo.length === 0) continue;

    // Tickets únicos que passaram por qualquer status do grupo
    const ticketsDoGrupo = new Set();
    for (const [, v] of statusDoGrupo) v.ticketsUnicos.forEach(t => ticketsDoGrupo.add(t));
    const grpUnico = ticketsDoGrupo.size;

    // Soma do SLA de todos os status do grupo
    const grpSlaSoma = statusDoGrupo.reduce((a, [s]) => a + (SLA_DIAS_UTEIS[s] || 0), 0);

    // Métricas agregadas
    const grpTotalOcorr = statusDoGrupo.reduce((a, [, v]) => a + v.ticketsUnicos.size, 0);
    const grpDias = statusDoGrupo.reduce((a, [, v]) => a + v.totalDias, 0);
    const grpDentro = statusDoGrupo.reduce((a, [, v]) => a + v.dentroSla, 0);
    const grpMediaDias = grpTotalOcorr > 0 ? grpDias / grpTotalOcorr : 0;
    const grpSla = grpTotalOcorr > 0 ? Math.round(grpDentro / grpTotalOcorr * 100) : 0;
    const grpFreq = total > 0 ? Math.round(grpUnico / total * 100) : 0;
    const grpColor = grpSla >= 80 ? C.green : grpSla >= 50 ? C.orange : C.red;

    rows += `<tr style="background:rgba(255,255,255,0.05);">
      <td style="font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:13px;color:var(--yellow);">${grupo}</td>
      <td class="num" style="font-weight:600;">${grpMediaDias.toFixed(2)}</td>
      <td class="num"><span style="color:${grpColor};font-weight:600;">${grpSla}%</span></td>
      <td class="num" style="font-weight:600;">${grpSlaSoma > 0 ? grpSlaSoma.toFixed(2) : '—'}</td>
      <td class="num">${grpUnico}</td>
      <td class="num">${grpFreq}%</td>
    </tr>`;

    // Linhas de sub-status
    for (const [status, v] of statusDoGrupo.sort((a, b) => b[1].ticketsUnicos.size - a[1].ticketsUnicos.size)) {
      const unicos = v.ticketsUnicos.size;
      const mediaDias = unicos > 0 ? v.totalDias / unicos : 0;
      const pctSla = unicos > 0 ? Math.round(v.dentroSla / unicos * 100) : 0;
      const freq = total > 0 ? Math.round(unicos / total * 100) : 0;
      const slaDias = SLA_DIAS_UTEIS[status] || null;
      const color = pctSla >= 80 ? C.green : pctSla >= 50 ? C.orange : C.red;
      rows += `<tr>
        <td style="padding-left:20px;color:var(--gray2);">${status}</td>
        <td class="num">${mediaDias.toFixed(2)}</td>
        <td class="num"><span style="color:${color};">● ${pctSla}%</span></td>
        <td class="num">${slaDias !== null ? slaDias : '—'}</td>
        <td class="num">${unicos}</td>
        <td class="num">${freq}%</td>
      </tr>`;
    }
  }

  tblEl.innerHTML = rows || `<tr><td colspan="6" style="text-align:center;color:var(--gray2);">
    ${Object.keys(deskHistory).length === 0 ? '⏳ Carregando histórico de tickets...' : 'Sem dados para o período/filtro selecionado'}
  </td></tr>`;

  // ── Histograma de lead time por macro status ──
  const filtroHistGrupo = document.getElementById('filtro-hist-grupo')?.value || 'todos';
  
  // Buckets em dias úteis: 0-1, 1-2, 2-4, 4-6, 6-8, 8-10, 10-12, 12-14, 14-16, 16-18, 18-20, 20-22, 22-24, 24-26, 26-28, 28-30, 30-32, 32-34, 34-36, 36-38, 38-40, 40-48, 48-100, 100+
  ...
  function getBucket(dias) {
    ...
    if (dias < 16) return 8;
    if (dias < 18) return 9;
    if (dias < 20) return 10;
    if (dias < 22) return 11;
    if (dias < 24) return 12;
    if (dias < 26) return 13;
    if (dias < 28) return 14;
    if (dias < 30) return 15;
    if (dias < 32) return 16;
    if (dias < 34) return 17;
    if (dias < 36) return 18;
    if (dias < 38) return 19;
    if (dias < 40) return 20;
    if (dias < 48) return 21;
    if (dias < 100) return 22;
    return 23;
  }

  const grupos_hist = filtroHistGrupo === 'todos'
    ? ['ENTRADA', 'AVALIAÇÃO', 'PAGAMENTO', 'TÉCNICO', 'DEVOLUÇÃO']
    : [filtroHistGrupo];

  const histData = {}; // { grupo: [count per bucket] }
  for (const g of grupos_hist) histData[g] = new Array(bucketLabels.length).fill(0);

  for (const h of histEntries) {
    for (const [status, hours] of Object.entries(h.statusTimes || {})) {
      const grupo = STATUS_GRUPOS[status];
      if (!grupo || !histData[grupo]) continue;
      const dias = horasParaDiasUteis(hours, h.createdTime);
      histData[grupo][getBucket(dias)]++;
    }
  }

  const histColors = { ENTRADA: C.green, AVALIAÇÃO: C.blue, PAGAMENTO: '#e0c052', TÉCNICO: C.orange, DEVOLUÇÃO: C.purple };
  const histCanvasIds = { ENTRADA: 'chart-hist-entrada', AVALIAÇÃO: 'chart-hist-avaliacao', PAGAMENTO: 'chart-hist-pagamento', TÉCNICO: 'chart-hist-tecnico', DEVOLUÇÃO: 'chart-hist-devolucao' };
  const histGruposAll = ['ENTRADA', 'AVALIAÇÃO', 'PAGAMENTO', 'TÉCNICO', 'DEVOLUÇÃO'];

  for (const g of histGruposAll) {
    const canvasId = histCanvasIds[g];
    if (!document.getElementById(canvasId)) continue;
    const dados = histData[g] || new Array(bucketLabels.length).fill(0);
    // Remove buckets vazios do final para deixar gráfico mais limpo
    let lastNonZero = dados.length - 1;
    while (lastNonZero > 3 && dados[lastNonZero] === 0) lastNonZero--;
    const dadosTrimmed = dados.slice(0, lastNonZero + 2);
    const labelsTrimmed = bucketLabels.slice(0, lastNonZero + 2);
    ...
    makeChart(canvasId, {
      ...
      data: {
        ...
        datasets: [{
          label: g,
          data: dadosTrimmed,
          backgroundColor: histColors[g] || C.gray,
          borderRadius: 3
        }]
      },
      options: {
  ...
  // Gráfico % SLA por mês
  const slaByMonth = {}; // { 'YYYY-MM': { ENTRADA: {dentro,total}, AVALIAÇÃO: ... } }
  for (const h of histEntries) {
    ...
    for (const [status, hours] of Object.entries(h.statusTimes || {})) {
      const grupo = STATUS_GRUPOS[status];
      if (!grupo || !slaByMonth[month][grupo]) continue;
      const dias = horasParaDiasUteis(hours, h.createdTime);
      const slaDias = SLA_DIAS_UTEIS[status];
      slaByMonth[month][grupo].t++;
      if (!slaDias || dias <= slaDias) slaByMonth[month][grupo].d++;
    }
  }

  const months = Object.keys(slaByMonth).sort();
  const grupoColors = { ENTRADA: C.green, AVALIAÇÃO: C.blue, TÉCNICO: C.orange, PAGAMENTO: '#e0c052', DEVOLUÇÃO: C.purple };

  makeChart('chart-sla-mensal', {
    type: 'bar',
    data: {
      labels: months.map(m => {
        const [y, mo] = m.split('-');
      ...
      datasets: [
        ...grupos.map(g => ({
          label: g,
          data: months.map(m => {
            const v = slaByMonth[m]?.[g];
            return v && v.t > 0 ? Math.round(v.d/v.t*100) : null;
          }),
          backgroundColor: grupoColors[g],
          borderRadius: 3
        })),
        {
          label: 'Meta (80%)',
    ...
    options: {
      ...
      scales: {
        ...
        y: { beginAtZero: true, max: 105, ticks: { callback: v => v + '%' } }
      }
    }
  });

  // KPI histórico — usa tickets únicos por status
  const allDentro = Object.values(statusMetrics).reduce((a, v) => a + v.dentroSla, 0);
  const allTotal = Object.values(statusMetrics).reduce((a, v) => a + v.ticketsUnicos.size, 0);
  const pctGeral = allTotal > 0 ? Math.round(allDentro/allTotal*100) : 0;
  const elSlaGeral = document.getElementById('kpi-sla-geral');
  if (elSlaGeral) {
    const color = pctGeral >= 80 ? C.green : pctGeral >= 50 ? C.orange : C.red;
    elSlaGeral.innerHTML = `<div class="val" style="color:${color}">${pctGeral}%</div><div class="lbl">% Conclusão SLA Geral</div>`;
  }
  const elSlaTickets = document.getElementById('kpi-sla-tickets');
  if (elSlaTickets) elSlaTickets.innerHTML = `<div class="val">${total}</div><div class="lbl">Tickets com Histórico</div>`;
  const elSlaLoading = document.getElementById('kpi-sla-loading');
  if (elSlaLoading) {
    const totalFechados = deskTickets.filter(t => t.closedTime).length;
    elSlaLoading.innerHTML = `<div class="val" style="font-size:16px;">${Object.keys(deskHistory).length}/${totalFechados}</div><div class="lbl">Histórico Carregado</div>`;
  }
}
  ...
  while (hasMore) {
    try {
      ...
      document.getElementById('last-update').textContent =
        `⏳ Desk: ${deskTickets.length} tickets carregados...`;
      if (data.tickets.length > 0) loadReport();
    } catch { break; }
  }

  deskLoaded = true;
  await loadReport(); // espera RMA carregar antes de iniciar histórico
  document.getElementById('last-update').textContent = 'Atualizado: ' + new Date().toLocaleTimeString('pt-BR');
  // Inicia busca de histórico — agora _rmaRaw já está disponível
  fetchDeskHistory(deskTickets);
}

// ─── Load report (processa dados com tickets já carregados) ───────────
async function loadReport() {
  ...
  try {
    const res = await fetch('/api/report', {
      ...
      body: JSON.stringify({ from, to, tickets: deskTickets })
    });
    const data = await res.json();
    state.report = data;
    if (deskLoaded) {
    ...
    // Salva RMA raw para cruzamento com histórico de garantia
  ...
  renderAll(data);
    renderSlaReport(); // atualiza SLA com dados disponíveis
  } catch (e) { console.error('Erro ao carregar report:', e); }
}

async function loadData() {
  await loadReport();       // Carrega RMA/Spare Parts imediatamente (responde ao filtro de data)
  // Só busca Desk e histórico na primeira carga — filtro de data não reinicia isso
  if (!deskLoaded && deskTickets.length === 0) {
    fetchDeskPages();
  } else if (deskLoaded) {
    // Desk já carregado — só re-renderiza o SLA com o filtro de data atual
    renderSlaReport();
  }
}

// Salva raw RMA para cruzamento com histórico
function saveRmaRaw(reportData) {
  // Reconstruímos o raw a partir dos dados processados disponíveis
  window._rmaRaw = window._rmaRaw || [];
}
...
function renderProdutos(d) {
  ...
  document.getElementById('top-products-list').innerHTML = r.topProducts.map(p => `
    <div class="hbar-item">
      <div class="label"><span style="font-size:11px;">${p.model}</span><strong>${p.count}</strong></div>
  ...
  document.getElementById('tbody-rma').innerHTML = (d.rma._raw || []).slice(0, 200).map(r => {
    ...
    return `<tr>
      <td>${r.fornecedor}</td>
      <td style="max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${r.model}">${r.model}</td>
      <td>${r.sku}</td>
  ...
  // ── Gráficos de serviço por período (trimestral) ──
  // Usa dados calculados no backend — tem o campo service correto
  const trimestralRaw = d.rma?.trimestral || {};
  console.log('trimestralRaw keys:', Object.keys(trimestralRaw).slice(0,3), 'sample:', JSON.stringify(Object.values(trimestralRaw)[0]));

  function getServicoCategoria(service) {
    const s = (service || '').trim();
    if (s === 'Flawless' || s === 'Firmware Update') return 'A - Sem Defeito';
    if (s === 'Analyze' || s === 'Software upgrade') return 'B - Sem Serviço';
    ...
    return 'B - Sem Serviço';
  }

  const servicosCats = ['A - Sem Defeito','B - Sem Serviço','C - Recarga Bateria','D - Subst. PCB','E - Subst. Componentes','F - Troca de Produto'];
  const servicoCores = ['#52c878', '#5299e0', '#e09052', '#FFD04C', '#9b59e0', '#e05252'];

  const trimestreData = {};
  const garantiaTrimestre = {};

  for (const [trim, data] of Object.entries(trimestralRaw)) {
    trimestreData[trim] = {};
    garantiaTrimestre[trim] = {
      warranty: data.warranty || 0,
      noWarranty: data.noWarranty || 0,
      maintenance: data.maintenance || 0
    };
    for (const [svc, count] of Object.entries(data.services || {})) {
      const cat = getServicoCategoria(svc);
      trimestreData[trim][cat] = (trimestreData[trim][cat] || 0) + count;
    }
  }

  const trimestres = Object.keys(trimestreData).sort((a, b) => {
    // Suporta formato "T1 2025" (trimestre) e "01/2025" (mês)
    if (a.includes('/') && b.includes('/')) {
      const [ma, ya] = a.split('/'); const [mb, yb] = b.split('/');

  // Gráfico NeoSolar — barras empilhadas por período
  // Gráfico Local de Teste — barras empilhadas por período
  const locTrimestres = Object.keys(trimestralRaw).sort((a, b) => {
    if (a.includes('/') && b.includes('/')) {
      const [ma, ya] = a.split('/'); const [mb, yb] = b.split('/');
  ...
  const allLocations = [...new Set(
    Object.values(trimestralRaw).flatMap(t => Object.keys(t._locations || {}))
  )].sort();
  const locCores = [C.yellow, C.blue, C.green, C.orange, C.purple, C.red, C.gray2];

  makeChart('chart-test-location', {
    type: 'bar',
    data: {
      labels: locTrimestres,
      datasets: allLocations.map((loc, i) => ({
        label: loc,
        data: locTrimestres.map(t => trimestralRaw[t]?._locations?.[loc] || 0),
        backgroundColor: locCores[i % locCores.length],
        borderRadius: 2
      }))
    },
    options: {
      ...
      scales: {
        x: { stacked: true, ticks: { color: C.gray2, maxRotation: 45 } },
        y: { stacked: true, beginAtZero: true, ticks: { color: C.gray2 } }
      }
    },
    plugins: [{
      id: 'nsTotals',
  ...
  const servicoDatasets = (mode) => servicosCats.map((cat, i) => ({
    label: cat,
    data: trimestres.map(t => {
      if (mode === 'pct') {
        const total = Object.values(trimestreData[t] || {}).reduce((s, v) => s + v, 0);
        return total > 0 ? Math.round((trimestreData[t]?.[cat] || 0) / total * 100) : 0;
      }
      return trimestreData[t]?.[cat] || 0;
    }),
    backgroundColor: servicoCores[i],
    borderColor: servicoCores[i],
    borderRadius: 2,
    stack: 'stack'
  }));

  // Chart: serviços por período absoluto
  makeChart('chart-servico-abs', {
    type: 'bar',
    data: { labels: trimestres, datasets: servicoDatasets('abs') },
    options: {
      maintainAspectRatio: false,
      plugins: { legend: { position: 'right', labels: { boxWidth: 12, font: { size: 11 }, color: C.white } } },
      scales: {
        x: { stacked: true, ticks: { color: C.gray2, maxRotation: 45 } },
        y: { stacked: true, beginAtZero: true, ticks: { color: C.gray2 } }
      }
  ...
  makeChart('chart-servico-pct', {
    type: 'bar',
    data: { labels: trimestres, datasets: servicoDatasets('pct') },
    options: {
      maintainAspectRatio: false,
      plugins: { legend: { position: 'right', labels: { boxWidth: 12, font: { size: 11 }, color: C.white } } },
      scales: {
        x: { stacked: true, ticks: { color: C.gray2, maxRotation: 45 } },
        y: { stacked: true, max: 100, ticks: { color: C.gray2, callback: v => v + '%' } }
      }
    },
    plugins: [{
      id: 'segmentLabelsPct',
    ...
    // Totais de garantia no período
  const totalW = Object.values(garantiaTrimestre).reduce((s, v) => s + v.warranty, 0);
  const totalNW = Object.values(garantiaTrimestre).reduce((s, v) => s + v.noWarranty, 0);
  const totalM = Object.values(garantiaTrimestre).reduce((s, v) => s + v.maintenance, 0);
  const totalG = totalW + totalNW + totalM;

  // centerTextPlugin registrado globalmente

  makeChart('chart-garantia-donut', {
    type: 'doughnut',
    data: {
      labels: [
        `Garantia (${totalG > 0 ? Math.round(totalW/totalG*100) : 0}%)`,
        `Sem Garantia (${totalG > 0 ? Math.round(totalNW/totalG*100) : 0}%)`,
        `Manutenção Paga (${totalG > 0 ? Math.round(totalM/totalG*100) : 0}%)`
      ],
      datasets: [{ data: [totalW, totalNW, totalM], backgroundColor: [C.yellow, C.green, C.orange], borderWidth: 2, borderColor: C.bg2 }]
    },
    options: {
      maintainAspectRatio: false,
      cutout: '60%',
      centerText: { line1: totalG.toString(), line2: 'Total de Chamados' },
      plugins: {
  ...
  if (legendEl) {
    const labels = [
      { label: 'Garantia', val: totalW, pct: Math.round(totalW/totalG*100)||0, color: C.yellow },
      { label: 'Sem Garantia', val: totalNW, pct: Math.round(totalNW/totalG*100)||0, color: C.green },
      { label: 'Manutenção Paga', val: totalM, pct: Math.round(totalM/totalG*100)||0, color: C.orange }
    ];
    legendEl.innerHTML = labels.map(l =>
      `<span style="display:inline-flex;align-items:center;gap:5px;margin:0 10px;">
        <span style="width:10px;height:10px;border-radius:2px;background:${l.color};display:inline-block;"></span>
        <span style="font-size:11px;color:var(--gray2);">${l.label} (${l.pct}% — ${l.val})</span>
      </span>`
    ).join('');
  }

  makeChart('chart-garantia-periodo', {
    type: 'bar',
    data: {
      labels: trimestres,
      datasets: [
        { label: 'Garantia', data: trimestres.map(t => garantiaTrimestre[t]?.warranty || 0), backgroundColor: C.yellow, borderRadius: 2, stack: 'g' },
        { label: 'Sem Garantia', data: trimestres.map(t => garantiaTrimestre[t]?.noWarranty || 0), backgroundColor: C.green, borderRadius: 2, stack: 'g' },
        { label: 'Manutenção Paga', data: trimestres.map(t => garantiaTrimestre[t]?.maintenance || 0), backgroundColor: C.orange, borderRadius: 2, stack: 'g' }
      ]
    },
    options: {
      maintainAspectRatio: false,
      plugins: {
        ...
        tooltip: {
          callbacks: {
            afterBody: (items) => {
              const t = trimestres[items[0].dataIndex];
              const tot = (garantiaTrimestre[t]?.warranty||0) + (garantiaTrimestre[t]?.noWarranty||0) + (garantiaTrimestre[t]?.maintenance||0);
              const pct = tot > 0 ? Math.round(items[0].raw/tot*100) : 0;
              return `${pct}% do total`;
            }
          }
        }
    ...
    plugins: [{
      ...
      afterDatasetsDraw(chart) {
        ...
        data.datasets.forEach((ds, di) => {
          ...
          meta.data.forEach((bar, i) => {
            const val = ds.data[i];
            if (!val || val < 3) return; // não mostra se muito pequeno
            const barHeight = Math.abs(bar.base - bar.y);
            if (barHeight < 14) return; // não mostra se barra muito pequena
...
function abrirValidacao() {
  const painel = document.getElementById('painel-validacao');
  ...
  const statusList = [...new Set(
    Object.values(deskHistory).flatMap(h => Object.keys(h.statusTimes || {}))
  )].sort();
...
function renderValidacaoTabela() {
  ...
  for (const h of Object.values(deskHistory)) {
    ...
    for (const [status, hours] of Object.entries(h.statusTimes || {})) {
      ...
      const dias = horasParaDiasUteis(hours, h.createdTime);
      const slaDias = SLA_DIAS_UTEIS[status] || null;
      const dentroSla = !slaDias || dias <= slaDias;
  ...
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--gray2);padding:20px;">Nenhum dado encontrado</td></tr>';
    return;
  }

  tbody.innerHTML = rows.slice(0, 500).map(r => {
    const cor = r.dentroSla ? 'var(--green)' : 'var(--red)';
    const resultado = r.dentroSla ? '✓ OK' : '✗ Vencido';
    const dataStr = r.createdTime ? new Date(r.createdTime).toLocaleDateString('pt-BR') : '—';
    return `<tr>
      <td><strong>#${r.ticket}</strong></td>
      <td>${r.status}</td>
      <td style="color:var(--yellow);font-weight:600;">${r.grupo}</td>
      <td class="num">${r.hours.toFixed(1)}h</td>
      <td class="num">${r.dias.toFixed(2)}</td>
      <td class="num">${r.slaDias ?? '—'}</td>
      <td class="num" style="color:${cor};font-weight:600;">${resultado}</td>
      <td>${dataStr}</td>
    </tr>`;
  }).join('');
}
...
function renderOperacao(d) {
  // Cruza deskHistory com RMA para calcular custo por ticket
  // Mapa de deskNum -> dados RMA (inclui rmaRaw e rmaRawFull)
  const rmaByDesk = {};
  const allRma = window._rmaRawFull || window._rmaRaw || [];
  for (const r of allRma) {
    const num = String(r.deskNum || '').trim();
    if (num && num !== '0') rmaByDesk[num] = r;
  }
  console.log('rmaByDesk size:', Object.keys(rmaByDesk).length, 'sample keys:', Object.keys(rmaByDesk).slice(0,3));
  console.log('rmaRaw sample:', allRma.slice(0,2));
  console.log('deskHistory sample ticketNumbers:', Object.values(deskHistory).slice(0,3).map(h => h.ticketNumber));

  const from = new Date(document.getElementById('date-from').value);
  const to = new Date(document.getElementById('date-to').value);
  to.setHours(23,59,59);

  // Calcula custo por ticket
  const ticketsCusto = [];
  ...
  for (const h of Object.values(deskHistory)) {
    const d2 = new Date(h.createdTime);
    if (d2 < from || d2 > to) continue;

    let horasTicket = 0;
    const statusCustoList = [];

    for (const [status, hours] of Object.entries(h.statusTimes || {})) {
      if (!STATUS_CUSTO.has(status)) continue;
      // Calcula horas úteis reais para esse status
      const inicio = new Date(h.createdTime);
      const fim = new Date(inicio.getTime() + hours * 3600000);
      const hu = horasUteisEntre(inicio, fim);
      horasTicket += hu;
      statusCustoList.push(status);
    }

    if (horasTicket <= 0) continue;

    const custo = horasTicket * CUSTO_POR_HORA;
    totalHorasUteis += horasTicket;
    totalCusto += custo;

    // Dados do RMA para cruzamento
    const rma = rmaByDesk[String(h.ticketNumber)] || {};
    const linha = rma.fornecedor || 'Não identificado';
    const produto = rma.model || 'Não identificado';
    const garantia = rma.validation || '—';

    // Custo por fornecedor/linha
    custoPorLinha[linha] = (custoPorLinha[linha] || 0) + custo;

    // Custo por produto (modelo completo truncado)
    const prod = produto.length > 50 ? produto.substring(0, 50) + '...' : produto;
    custoPorProduto[prod] = (custoPorProduto[prod] || 0) + custo;

    // Período
    const mes = d2.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' });
    custoPorPeriodo[mes] = (custoPorPeriodo[mes] || 0) + custo;

    ticketsCusto.push({
      ticket: h.ticketNumber,
      produto: produto.length > 50 ? produto.substring(0, 50) + '...' : produto,
      fornecedor: linha,
      garantia,
      horasUteis: horasTicket,
      custo,
      statusList: [...new Set(statusCustoList)].join(', '),
      data: h.createdTime
    });
  }

  ticketsCusto.sort((a, b) => b.custo - a.custo);
  console.log('Custo debug:', {
    totalHistory: Object.keys(deskHistory).length,
    ticketsComCusto: ticketsCusto.length,
    totalCusto,
    sampleStatuses: Object.values(deskHistory).slice(0,2).map(h => Object.keys(h.statusTimes||{}))
  });

  // KPIs
  const ticketsComCusto = ticketsCusto.length;
  const custoMedio = ticketsComCusto > 0 ? totalCusto / ticketsComCusto : 0;
  document.getElementById('kpi-custo').innerHTML = [
    { val: `R$ ${totalCusto.toLocaleString('pt-BR', {minimumFractionDigits:0, maximumFractionDigits:0})}`, lbl: 'Custo Total Estimado', cls: 'red' },
    { val: ticketsComCusto, lbl: 'Tickets com Custo', cls: 'blue' },
    { val: `${totalHorasUteis.toFixed(1)}h`, lbl: 'Horas Úteis Técnicos', cls: 'yellow' },
    { val: `R$ ${custoMedio.toLocaleString('pt-BR', {minimumFractionDigits:0, maximumFractionDigits:0})}`, lbl: 'Custo Médio por Ticket', cls: 'orange' },
  ...
  // Chart: custo por linha
  const linhas = Object.entries(custoPorLinha).sort((a, b) => b[1] - a[1]).slice(0, 12);
  makeChart('chart-custo-linha', {
    type: 'bar',
    data: {
      labels: linhas.map(([l]) => l),
      datasets: [{ label: 'Custo R$', data: linhas.map(([,v]) => Math.round(v)), backgroundColor: palette(linhas.length), borderRadius: 4 }]
    },
    options: {
      maintainAspectRatio: false,
      indexAxis: 'y',
  ...
  // Chart: custo por período
  const periodos = Object.entries(custoPorPeriodo).sort((a,b) => a[0].localeCompare(b[0]));
  makeChart('chart-custo-periodo', {
    type: 'bar',
    data: {
      labels: periodos.map(([p]) => p),
      datasets: [{ label: 'Custo R$', data: periodos.map(([,v]) => Math.round(v)), backgroundColor: C.yellow, borderRadius: 4 }]
    },
    options: {
      maintainAspectRatio: false,
      plugins: { legend: { display: false } },
  ...
  const topProd = Object.entries(custoPorProduto).sort((a, b) => b[1] - a[1]).slice(0, 15);
  makeChart('chart-custo-produto', {
    type: 'bar',
    data: {
      labels: topProd.map(([p]) => p),
      datasets: [{ label: 'Custo R$', data: topProd.map(([,v]) => Math.round(v)), backgroundColor: C.orange, borderRadius: 4 }]
    },
    options: {
      maintainAspectRatio: false,
      indexAxis: 'y',
  ...
  document.getElementById('tbody-custo-tickets').innerHTML = ticketsCusto.slice(0, 200).map(t => {
    const garantiaTxt = (t.garantia || '').toLowerCase().includes('no warranty maintenance') ? 'Manutenção Paga'
      : (t.garantia || '').toLowerCase().includes('no warranty') ? 'Fora Garantia' : 'Garantia';
    const garantiaCor = garantiaTxt === 'Garantia' ? C.yellow : garantiaTxt === 'Fora Garantia' ? C.green : C.orange;
    const data = t.data ? new Date(t.data).toLocaleDateString('pt-BR') : '—';
    return `<tr>
      <td><strong>#${t.ticket}</strong></td>
      <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${t.produto}">${t.produto}</td>
      <td>${t.fornecedor}</td>
      <td style="color:${garantiaCor};font-weight:600;">${garantiaTxt}</td>
      <td class="num">${t.horasUteis.toFixed(1)}h</td>
      <td class="num" style="color:var(--yellow);font-weight:600;">R$ ${fmt(t.custo)}</td>
      <td style="font-size:10px;color:var(--gray2);">${t.statusList}</td>
      <td>${data}</td>
    </tr>`;
...
// ─── OPERAÇÃO (legado — não usado) ───────────────────────────────────
function _renderOperacaoLegado(d) {
  const desk = d.desk;
  ...
  const historico = desk.totalHistorico || 0;
  const fechados = desk.totalClosed || 0;

  document.getElementById('kpi-operacao').innerHTML = [
    { val: totalDesk, lbl: 'Tickets Abertos Agora', cls: 'blue' },
    { val: historico, lbl: 'Tickets no Período', cls: 'yellow' },
    { val: fechados, lbl: 'Fechados no Período', cls: 'green' },
  ...
  // Chart: tempo médio histórico por status
  const avgHist = (desk.avgTimeByStatus || []).filter(a => a.avgHours > 0).slice(0, 12);
  makeChart('chart-avg-status-hist', {
    type: 'bar',
    data: {
      labels: avgHist.map(a => a.status.replace('Aguardando', 'Ag.').replace('Manutenção', 'Manut.').replace('tratativa p/ devolução cliente', 'Devolução')),
      datasets: [
        { label: 'Média Real (h)', data: avgHist.map(a => parseFloat(a.avgHours.toFixed(1))), backgroundColor: C.yellow, borderRadius: 4 },
        { label: 'SLA (h)', data: avgHist.map(a => a.sla || 0), backgroundColor: 'rgba(224,82,82,0.3)', borderRadius: 4 }
      ]
    },
    options: {
      indexAxis: 'y',
  ...
  const trend = desk.monthlyTrend || {};
  const trendMonths = Object.keys(trend).sort();
  makeChart('chart-monthly-trend', {
    type: 'bar',
    data: {
      labels: trendMonths,
      datasets: [
        { label: 'Abertos', data: trendMonths.map(m => trend[m].opened), backgroundColor: C.blue, borderRadius: 3 },
        { label: 'Fechados', data: trendMonths.map(m => trend[m].closed), backgroundColor: C.green, borderRadius: 3 }
      ]
    },
    options: { plugins: { legend: { position: 'bottom' } }, scales: { y: { beginAtZero: true } } }
  });

  // SLA by status table (atual)
  ...
  // Tech histórico table
  const techHist = (desk.avgTimeByAgent || []).slice(0, 10);
  const elTechHist = document.getElementById('tbody-tech-hist');
  if (elTechHist) {
    elTechHist.innerHTML = techHist.map(t => {
      const resolvePct = t.count > 0 ? Math.round(t.closed/t.count*100) : 0;
      const cls = resolvePct >= 60 ? 'ok' : resolvePct >= 30 ? 'atencao' : 'vencido';
      return `<tr>
        <td style="color:var(--yellow);font-weight:600;">${t.name}</td>
        <td class="num">${t.count}</td>
        <td class="num" style="color:var(--green)">${t.closed}</td>
        <td class="num">${t.avgHours > 0 ? t.avgHours.toFixed(1)+'h' : '—'}</td>
        <td><div class="prog-bar"><div class="fill ${cls}" style="width:${resolvePct}%"></div></div>
            <div style="font-size:11px;color:var(--gray2);margin-top:2px;">${resolvePct}% resolvidos</div></td>
      </tr>`;
    }).join('') || '<tr><td colspan="5" style="text-align:center;color:var(--gray2);">Sem dados históricos</td></tr>';
  }

  // Tech SLA atual table
...
function renderSpareParts() {
  ...
  document.getElementById('tbody-spare').innerHTML = filtered.map(p => {
    ...
    const alert = p.totalFisico <= min && min > 0;
    ...
    return `<tr class="${alert ? 'alert-row' : ''}">
      ...
      <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${p.modelo}">${p.modelo}</td>
      <td class="num">${p.quantidade}</td>
      <td class="num">${p.saida}</td>
      <td class="num" style="font-weight:600;">${p.totalFisico}</td>
...
// ─── INIT ─────────────────────────────────────────────────────────────
setPeriodAll(); // Carrega tudo por padrão
loadData();     // Carrega automaticamente ao abrir a página
</script>
</body>
</html>
